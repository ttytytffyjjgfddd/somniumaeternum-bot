...
def start_command(message):
    ...
    with sqlite3.connect("database.db") as db:  # 3 підключення
        ...
        if user and user[3]:  # перевірка реєстрації
            bot.reply_to(
                message,
                "_Ви вже зареєстровані. Перегляньте асортимент._",
                parse_mode="Markdown",
            )
            ...
            bot.reply_to(
                message,
                "_Будь ласка, введіть ваш номер телефону для реєстрації. Обов'язково у форматі +380._",
                parse_mode="Markdown",
            )
...
def handle_full_name(message, phone):
    ...
    with sqlite3.connect("database.db") as db:  # 4 підключення
        cursor = db.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO users (id, phone, full_name, is_registered) VALUES (?, ?, ?, ?)",
            (user_id, phone, full_name, 1),
        )
        db.commit()

    bot.reply_to(
        message, "_Реєстрація успішна! Перегляньте асортимент._", parse_mode="Markdown"
    )
    show_main_menu(message)
...
def show_main_menu(message):
    ...
    keyboard.add(
        types.KeyboardButton("Асортимент"),
        types.KeyboardButton("Розмірна сітка"),
        types.KeyboardButton("Розмірна сітка для взуття"),
        types.KeyboardButton("Умови доставки"),
    )
    bot.send_message(
        message.chat.id,
        "_Оберіть опцію:_",
...
def show_catalog_options(message):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(
        types.KeyboardButton("ВЕРХ"),
        types.KeyboardButton("НИЗ"),
        types.KeyboardButton("ВЗУТТЯ"),
        types.KeyboardButton("АКСЕСУАРИ"),
    )
    bot.send_message(
        message.chat.id,
        "_Оберіть категорію асортименту:_",
        reply_markup=keyboard,
        parse_mode="Markdown",
    )
...
# вибір категорії
@bot.message_handler(
    func=lambda message: message.text in ["ВЕРХ", "НИЗ", "ВЗУТТЯ", "АКСЕСУАРИ"]
)
def show_products(message):
        ...
        if products:
            for product in products:
                ...
                order_button = types.InlineKeyboardButton(
                    "🛒 ЗАМОВИТИ", callback_data=f"order_{product[0]}"
                )  # product[0] - айді товару
    ...
    bot.send_message(
        message.chat.id,
        "_Натисніть кнопку нижче, щоб повернутись._",
        reply_markup=keyboard,
        parse_mode="Markdown",
    )
...
@bot.message_handler(
    func=lambda message: message.text == "🔙 Повернутись назад до меню"
)
...
def handle_order(call):
    ...
    bot.send_message(
        call.from_user.id,
        f"Ви замовили товар з ID: {product_id}. Очікуйте на зворотній зв'язок від менеджера. Дякуємо.",
    )
